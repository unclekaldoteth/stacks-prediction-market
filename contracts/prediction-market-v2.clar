...
(define-public (place-bet (round-id uint) (direction uint) (amount uint))
    (let
        ...
        (asserts! (>= amount MIN_BET) ERR_INVALID_AMOUNT)  ;; Minimum 1 STX
...
(define-public (claim-winnings (round-id uint))
    (let
        (
            (claimer tx-sender)  ;; CRITICAL: Store user principal BEFORE as-contract context
            (round (unwrap! (map-get? rounds round-id) ERR_ROUND_NOT_FOUND))
        ...
        (asserts! (> winning-pool u0) ERR_NO_WINNINGS)  ;; Prevent division by zero
